...
class RawOutputWebsiteBuilder:
    ...
    def __get_dataset_results_as_html(self, dataset_name, csv_results_path, show_figures=False):
        ...
        fig = make_subplots(
            ...
            subplot_titles=("Position", "3D Trajectory", "Orientation", "Gyro Bias", "Velocity", "Accel Bias"),
            shared_xaxes=True,
            vertical_spacing=0.1,
        )

        fig.update_layout(title_text="Raw VIO Output for dataset: %s" % dataset_name, template="plotly_white")
    ...
    def __plot_multi_line(self, df, x_id, y_ids, fig=None, row=None, col=None):
        ...
        for y_id in y_ids:
            assert y_id in df
            fig.add_trace(go.Scatter(x=df[x_id], y=df[y_id], mode=mode, name=y_id), row=row, col=col)
        return fig
